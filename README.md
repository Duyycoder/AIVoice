...
| `--engine` | Không | Chọn động cơ sinh/chuyển đổi giọng nói (`edge`, `piper`, `clone`, `rvc`). | Lấy từ `configs/default.json` |
...
* **`chay_giao_dien.bat`**: Mở nhanh máy chủ Web UI cục bộ và tự động khởi động trình duyệt.
* **`kiem_tra_gpu.bat`**: Chạy nhanh báo cáo chẩn đoán GPU/CUDA.
* **`chay_kiem_thu.bat`**: Chạy nhanh bộ kiểm thử tích hợp (tests suite) để xác minh tính ổn định của hệ thống.
