...
# Apply PyTorch/Torchao compatibility patches before any torch-dependent imports
try:
    import importlib
    # Reset torch modules
    ...
    logger.warning(f"Failed to clear pre-existing monkey patches: {patch_ex}")
...
try:
    ...
    if _orig_reg is None or not hasattr(_orig_reg, "__is_patched__"):
        logger.info("Applying torch.utils._pytree.register_constant monkey-patch inside dubbing.py.")
    ...
    logger.warning(f"Failed to apply register_constant patch: {patch_ex}")
...
try:
    ...
    def _safe_lib_define(self, *args, **kwargs):
        try:
            ...
            if "multiple times" in str(e):
                return None
            raise
    _safe_lib_define.__is_patched__ = True
    torch.library.Library.define = _safe_lib_define
    ...
    def _safe_lib_impl(self, *args, **kwargs):
        try:
            ...
            if "already a kernel registered" in str(e):
                return None
            raise
    _safe_lib_impl.__is_patched__ = True
    ...
    logger.warning(f"Failed to apply Library/Decomp/Fake patches: {patch_ex}")
