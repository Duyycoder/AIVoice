...
def burn_subtitles_ffmpeg(video_path: str, subtitle_path: str, output_path: str, codec: str = None, audio_path: str = None) -> bool:
    ...
    If audio_path is provided, replaces the video's original audio track with the new audio track.
    Otherwise, copies original audio directly without re-encoding (-c:a copy).
    """
    import subprocess
    import shutil
    
    # 1. Resolve effective codec
    if not codec:
    ...
    abs_audio_path = os.path.abspath(audio_path) if audio_path else None
    
    # To avoid Windows path escaping bugs with ffmpeg filters (especially drive letters and colons),
    # we copy files to the task directory or run in the task directory using relative paths.
    work_dir = os.path.dirname(abs_output_path)
    if not work_dir:
        work_dir = os.getcwd()
        
    # We will copy input files temporarily or use relative paths from work_dir.
    # Let's compute relative paths from work_dir to the inputs.
    temp_files = []
    try:
        rel_video = os.path.relpath(abs_video_path, start=work_dir)
        rel_sub = os.path.relpath(abs_subtitle_path, start=work_dir)
        rel_output = os.path.relpath(abs_output_path, start=work_dir)
        rel_audio = os.path.relpath(abs_audio_path, start=work_dir) if abs_audio_path else None
    except ValueError:
        # On Windows, if files are on different drives, relpath will fail.
        # In that case, copy them to work_dir temporarily.
        logger.info("Files are on different drives. Copying to task directory for processing.")
        temp_video_name = "temp_input_video" + os.path.splitext(abs_video_path)[1]
        temp_sub_name = "temp_input_sub" + os.path.splitext(abs_subtitle_path)[1]
        shutil.copy2(abs_video_path, os.path.join(work_dir, temp_video_name))
        shutil.copy2(abs_subtitle_path, os.path.join(work_dir, temp_sub_name))
        rel_video = temp_video_name
        rel_sub = temp_sub_name
        ...
        if abs_audio_path:
            ...
            rel_audio = temp_audio_name
            temp_files.append(os.path.join(work_dir, temp_audio_name))
    ...
    logger.info(f"Running fast subtitle burn in {work_dir}: {' '.join(cmd)}")
        ...
        for f in temp_files:
            if os.path.exists(f):
                try:
                    ...
                    logger.warning(f"Failed to remove temp file {f}: {clean_err}")
                    
        if res.returncode == 0:
            logger.success("Fast subtitle burning completed successfully!")
            return True
        else:
            logger.warning(f"Fast subtitle burning failed with code {res.returncode}: {res.stderr}")
            # If h264_nvenc or other hardware codec failed, try falling back to CPU (libx264)
            if codec != "libx264":
                logger.info("Retrying with CPU codec (libx264)...")
                try:
                    c_v_idx = cmd.index("-c:v")
                    cmd[c_v_idx + 1] = "libx264"
                    res_retry = subprocess.run(cmd, cwd=work_dir, capture_output=True, text=True)
                    if res_retry.returncode == 0:
                        logger.success("Subtitle burning with libx264 succeeded!")
                        return True
                    else:
                        logger.error(f"Fallback subtitle burning failed: {res_retry.stderr}")
                except Exception as retry_err:
                    logger.error(f"Failed to retry with libx264: {retry_err}")
            return False
    except Exception as e:
        logger.error(f"Error during FFmpeg subtitle burning: {e}")
        # Clean up temp files
        for f in temp_files:
            if os.path.exists(f):
                try:
